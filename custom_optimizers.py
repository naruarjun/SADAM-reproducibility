...
class SC_SGD(torch.optim.Optimizer):
    def __init__(self, params, convex=False, lr=1e-2, eps=1e-9,
                 weight_decay=1e-2, correct_bias=True):

        if lr < 0.0:
            raise ValueError("Invalid LR {} Allowed >= 0.0".format(lr))
        if not eps >= 0.0:
            raise ValueError("Invalid epsilon {} Allowed >= 0.0".format(eps))

        defaults = dict(lr=lr, eps=eps, weight_decay=weight_decay,
                        correct_bias=correct_bias, convex=convex)
...
class SC_RMSprop(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-2, alpha=0.9, eps1=0.1, eps2=1,
                 weight_decay=1e-2, convex=False):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps1:
            raise ValueError("Invalid epsilon value: {}".format(eps1))
        if not 0.0 <= eps2:
            raise ValueError("Invalid epsilon value: {}".format(eps2))
        if not 0.0 <= weight_decay:
            raise ValueError("Invalid weight_decay: {}".format(weight_decay))
        if not 0.0 <= alpha:
            raise ValueError("Invalid alpha value: {}".format(alpha))

        defaults = dict(lr=lr, alpha=alpha, eps1=eps1, eps2=eps2,
                        weight_decay=weight_decay, convex=convex)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                eps_replaced = torch.exp(
                                -square_avg.mul(
                                    group['eps1']*state['step']
                                    )
                                )
...
class SC_Adagrad(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-2, eps1=0.1, eps2=1,
                 weight_decay=1e-2, convex=False):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= eps1:
